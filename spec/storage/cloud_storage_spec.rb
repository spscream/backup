...
module Backup
  describe Storage::CloudStorage do
    let(:model) { Model.new(:test_trigger, 'test label') }
    let(:storage) { Storage::CloudStorage.new(model) }
    let(:s) { sequence '' }
    ...
    describe '#initialize' do
      ...
      it 'configures the storage' do
        storage = Storage::CloudStorage.new(model, :my_id) do |nf|
          nf.keep           = 2
      ...
      it 'strips leading path separator' do
        storage = Storage::CloudStorage.new(model) do |s3|
          s3.path = '/this/path'
        end
    ...
    describe '#connection' do
      ...
      it 'creates a new connection' do
        Fog::Storage.expects(:new).with(
            :provider                 => 'Google',
            :google_storage_access_key_id  => 'my_storage_token',
            :google_storage_secret_access_key  => 'my_storage_secret'
        ).returns(connection)
    ...
    describe '#remove!' do
      ...
      it 'raises an error if the remote_path is missing' do
        ...
        expect do
          storage.send(:remove!, package)
        end.to raise_error(Storage::CloudStorage::Error) {|err|
                 expect( err.message ).to eq(
                                              'Storage::CloudStorage::Error: ' +
